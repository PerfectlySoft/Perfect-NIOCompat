	...
	public static func contentCompression(data: [String:Any]) throws -> HTTPResponseFilter {
		...
		struct CompressResponse: HTTPResponseFilter {
			...
			func filterHeaders(response: HTTPResponse, callback: (HTTPResponseFilterResult) -> ()) {
				...
				let contentLength = Int(response.header(.contentLength) ?? "0") ?? 0
				if !response.isStreaming && (response.bodyBytes.count < responseMinSizeNoCompression && contentLength < responseMinSizeNoCompression) {
					return callback(.continue)
				}
